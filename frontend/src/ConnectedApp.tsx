...
function PipelineDetail({ id, success, fail, refreshList }: { id: number; success: (m: string) => void; fail: (e: unknown) => void; refreshList: () => Promise<void> }) {
  ...
  async function deploy() { if (!item) return; setBusy(true); try { const result = await platformApi.deployPipeline(item.pipelineId); success(`Deployment #${result.deploymentId} · ${result.status}`); await Promise.all([load(), refreshList()]); } catch (e) { fail(e); } finally { setBusy(false); } }
  if (!item) return <Empty title="Pipeline 조회 중">상세 API 응답을 기다리고 있습니다.</Empty>;
  const deployable = item.pipelineType === 'CUSTOM_JAR' && ['ARTIFACT_UPLOADED', 'STOPPED', 'FAILED'].includes(item.pipelineStatus);
  return <><div className="welcome-row"><div><h2>{item.pipelineName}</h2><p>Pipeline #{item.pipelineId} · Owner #{item.ownerUserId}</p></div><div className="connected-heading-actions"><Status value={item.pipelineStatus} /><button className="primary-button compact" onClick={() => void deploy()} disabled={!deployable || busy}>배포 실행</button></div></div><form className="panel connected-create" onSubmit={save}><div className="panel-heading"><div><h3>기본 정보</h3><p>{item.pipelineType}</p></div><button className="secondary-button">변경 저장</button></div><div className="connected-form"><Field label="Pipeline 이름"><input value={name} onChange={(e) => setName(e.target.value)} required /></Field><Field label="Type"><input value={item.pipelineType} disabled /></Field><Field label="설명" wide><textarea rows={4} value={description} onChange={(e) => setDescription(e.target.value)} /></Field></div></form><section className="connected-availability"><h3>운영 기능 연결 상태</h3><p><b>연결됨</b> 상태 조회, 기본 정보 수정, CUSTOM_JAR 배포</p><p><span>API 필요</span> 중지, Deployment 이력, 결과 조회, 실패 분석</p></section></>;
}
