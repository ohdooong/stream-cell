...
public class SchemaValidator implements Validator<PipelinePlanValidationContext> {
    ...
    public void validate(PipelinePlanValidationContext context) {
        ...
        String timeField = sourceTopic.getTimeField();
        String valueType = (String) parsedTopicSchema.getOrDefault(timeField, "");
        if (!valueType.startsWith("TIMESTAMP(3)")) {
            throw new BaseAPIException(ErrorCode.INVALID_TIME_FILED);
        }
